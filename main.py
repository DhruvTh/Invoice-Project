...
import time, uvicorn, copy, json, traceback
        ...
        llm_input = LLMInput(
            ...
            history=[
                History(
                    role="user",
                    content=Content(
                        text="Please extract the document type. While extracting information, You must not make up any information by yourself. If you think that, given details are missing, you could mention false for any extracting parameter.",
                        image_data=[ImageData(url=img) for img in url_to_pil_image(input_data.invoice_url)]
                    )
                )],
                function_call_list=[copy.deepcopy(doc_type_schema)]
        )
        doc_identification_response, doc_identify_cost = llm_list[input_data.llm].generate_response(llm_input)

        if(doc_identification_response["is_purchase_order"]):
            po_extraction_input = LLMInput(
                llm = input_data.llm,
                llm_model=input_data.llm_model,
                history=[History(
                    role="user",
                ...
                function_call_list=[copy.deepcopy(po_extraction_schema)]
            )
            po_extraction_response, po_extraction_cost = llm_list[input_data.llm].generate_response(po_extraction_input)
            
            final_cost = sum_token_cost(po_extraction_cost.token_cost, doc_identify_cost.token_cost)

            output = TaskResponse(
                token_cost = final_cost,
                time_required=time.time()-st,
                extracted_invoice_data=po_extraction_response,
                doc_type="PO"
            )
            invoice_db.add_po(po_extraction_response)
            return JSONResponse(output.model_dump())
        elif(doc_identification_response["is_invoice"]):
            llm_input = LLMInput(
            llm = input_data.llm,
            llm_model=input_data.llm_model,
            history=[
            ...
            invoice_identify_cost.token_cost = sum_token_cost(invoice_identify_cost.token_cost, doc_identify_cost.token_cost)
            ...
            raise Exception("Given Document is not invoice or PO")
